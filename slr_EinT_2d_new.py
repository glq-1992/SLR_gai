...
class Discriminator(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, f,number_class):
        ...
        self.classifier_combine = nn.Linear(hidden_size*2, self.num_classes)
    ...
    def forward_classifier(self, x):
        ...
        x=self.pool(x.transpose(0,2)).squeeze(2)
        return self.f(self.map3(x.transpose(0,1)))
    def forward_ctc(self, x):
        x = self.classifier_combine(x)

        return x
    def forward_location(self, x):
        x = self.adapool(x).squeeze(-1)
...
class Generator(nn.Module):
    ...
    def forward(self, Tframewise_all,Eframewise_all,Tlen_x_all,Elen_x_all,):
        Tlen_x = torch.zeros_like(Tlen_x_all).cuda()  # 456-551放到一个对象里，当做生成器
        Elen_x = torch.zeros_like(Elen_x_all).cuda()
        # Clen_x = torch.zeros_like(Clen_x_all).cuda()
        batch=Tframewise_all.size(0)
        hand_start_T_list = []
        hand_end_T_list = []
        hand_start_E_list = []
        hand_end_E_list = []
        ...
        location_prediction = torch.zeros(batch,dtype=torch.int64).cuda()
        for i in range(batch):
            ...
            location_prediction[i] = index_action
        ...
        Comframewise = concatenated_feature_final
        ...
        length_T=Tlen_x+1
        return Comframewise, input_lengths_entire_loca,location_prediction,length_T
# class Generator(nn.Module):
#     def __init__(self, hidden_size):
#         super(Generator, self).__init__()
#         self.query = nn.Linear(hidden_size, hidden_size)
...
#         location_prediction = torch.zeros(batch,dtype=torch.int64).cuda()
#         for i in range(batch):
#             # com_feature_batch = torch.cat((Eframewise[i],Tframewise[i]), dim=0)
#             location = random.randint(0, (Tlen_x[i] - 1))
#             com_feature_batch = torch.cat((Tframewise[i][:location], Eframewise[i], Tframewise[i][location:]), dim=0)
#
#             length = com_feature_batch.size(0)
#             concatenated_feature_final[i, :length, :] = com_feature_batch
#         Comframewise = concatenated_feature_final
#         input_lengths_entire_com = []
#         for i in range(batch):
#             input_lengths_entire_com.append(Tlen_x[i] + Elen_x[i])
...
class SLRModel(nn.Module):
    def __init__(self, num_classes, num_classes_T,num_classes_E,c2d_type, conv_type, use_bn=False, tm_type='BiLSTM',
        ...
        self.D = Discriminator(input_size=1024,
                          hidden_size=512,
                          output_size=1,
                          f=discriminator_activation_function,
                          number_class=self.num_classes)
        self.G=Generator(hidden_size=512)
    def backward_hook(self, module, grad_input, grad_output):
        for g in grad_input:
            g[g != g] = 0

    def masked_bn(self, inputs, len_x,max_x):
        def pad(tensor, length):
            return torch.cat([tensor, tensor.new(length - tensor.size(0), *tensor.size()[1:]).zero_()])

        x = torch.cat([inputs[max(len_x) * idx:max(len_x) * idx + lgt] for idx, lgt in enumerate(len_x)])
        x = self.conv2d(x)
        x = torch.cat([pad(x[sum(max_x[:idx]):sum(max_x[:idx + 1])], max(max_x))
                       for idx, lgt in enumerate(max_x)])
    ...
    def cosine_similarity_onevideo(self, frames, input_lengths,right_pad=None, bound=20):
        ...
        # print('hand start{}  hand end{}  length of entity{}'.format(hand_start, hand_end, input_lengths.item()))
        return hand_start, hand_end
    def forward_train_true(self, Tx, Tlen_x_all, label=None, label_lgt=None):
        ...
        # a.append(Tlen_x_all)
        # a_T = torch.tensor(a)
        # Ttemp = torch.max(Tlen_x_all)
        # a_max = []
        # a_max.append(Ttemp)
        # a_T_max = torch.tensor(a_max)
        Tinputs = Tx.reshape(batch * Ttemp, channel, height, width)
        Tframewise_all = self.masked_bn(Tinputs, Tlen_x_all,Tlen_x_all)
        ...
        # conv_pred = self.decoder_combine.decode(conv1d_outputs_second['conv_logits'], lgt_second, batch_first=False,
        #                                         probs=False)
        # decision = self.D(tm_outputs_second['predictions'])
        # label_calss=Variable(torch.ones([1]))
        # label_d.append(label_calss)
        # decision=self.D(tm_outputs_second['predictions'])
        return {
            "framewise_features": Tframewise_all.transpose(1, 2),
            "visual_features": x_first,
            "feat_len": lgt_first,
            "conv_logits": conv1d_outputs_first['conv_logits'],
            "clipwise_features": tm_outputs_first['predictions'],
            # "sequence_logits": outputs,
            # "conv_sents": conv_pred,
            # "recognized_sents": pred,
            # "label_calss": label_d_new,
            # "decision": decision
        }
    def forward_train_false(self, Tx, Ex , Tlen_x_all, Elen_x_all, label=None, label_lgt=None):
        ...
        # b.append(Elen_x_all)
        # b_T = torch.tensor(b)
        # Etemp = torch.max(Elen_x_all)
        # b_max = []
        # b_max.append(Etemp)
        # b_T_max = torch.tensor(b_max)
        Einputs = Ex.reshape(batch * Etemp, channel, height, width)
        Eframewise_all = self.masked_bn(Einputs, Elen_x_all, Elen_x_all)
        Eframewise_all = Eframewise_all.reshape(batch, Etemp, -1)

        # Tlen_x = torch.zeros_like(Tlen_x_all).cuda()
        # Elen_x = torch.zeros_like(Elen_x_all).cuda()
        # # Clen_x=torch.zeros_like(Clen_x_all[i]).cuda()
        # hand_start_T_list = []
        # hand_end_T_list = []
        # hand_start_E_list = []
        # hand_end_E_list = []
        #
        # # for i in range(0,batch):
        # hand_start_T, hand_end_T = self.cosine_similarity_onevideo(Tframewise_all[0], Tlen_x_all)
        # hand_start_E, hand_end_E = self.cosine_similarity_onevideo(Eframewise_all[0], Elen_x_all)
        # hand_start_T_list.append(hand_start_T)
        # hand_start_E_list.append(hand_start_E)
        # hand_end_T_list.append(hand_end_T)
        # hand_end_E_list.append(hand_end_E)
        # Tlen_x = hand_end_T - hand_start_T
        # Elen_x = hand_end_E - hand_start_E
        # # Clen_x=Tlen_x+Elen_x
        #
        # Tframewise = []
        # Eframewise = []
        # # for i in range(0, batch):
        # Tframewise.append(Tframewise_all[0][hand_start_T_list[0]:hand_end_T_list[0]])
        # Eframewise.append(Eframewise_all[0][hand_start_E_list[0]:hand_end_E_list[0]])
        #
        # #### EinT
        # # e_length = torch.max(Elen_x)
        # # t_length = torch.max(Tlen_x)
        ...
        # concatenated_feature_final = torch.zeros(batch, c_length, 512).cuda()
        # # for i in range(batch):
        # com_feature_batch = torch.cat((Tframewise[0], Tframewise[0][-1, :].unsqueeze(0)), dim=0)
        # # print('t_len:',Tframewise[i].size(0))
        # mixed_query_layer = self.query(com_feature_batch)
        # mixed_key_layer = self.key(Eframewise[0])
        # mixed_value_layer = self.value(Eframewise[0])
        # attention_scores = torch.matmul(
        #     mixed_query_layer, mixed_key_layer.transpose(0, 1))
        # attention_scores = attention_scores / \
        #                    math.sqrt(512)
        ...
        # index_action = torch.max(action, 1)[1]
        # # print('predict:',index_action)
        # b = action[:, index_action[0]:index_action[0] + 1][0].repeat(Elen_x)
        # b = b.unsqueeze(0)
        # action_list = torch.cat([action[:, :index_action[0]], b, action[:, index_action[0] + 1:]], 1)  #### M'
        # action_list = action_list.squeeze()
        # action_clone_tensor = action_list
        # action_clone_tensor_t = torch.ones(Tlen_x + Elen_x).cuda() - action_clone_tensor  #### 1-M'
        #
        # concatenated_feature_t = torch.zeros(Tlen_x + Elen_x, 512).cuda()
        # concatenated_feature_e = torch.zeros(Tlen_x + Elen_x, 512).cuda()
        # concatenated_feature_e[action_list == True] = Eframewise[0]
        # concatenated_feature_t[action_list == False] = Tframewise[0]
        # concatenated_feature = torch.einsum('tc,t->tc', concatenated_feature_t,
        #                                     action_clone_tensor_t) + torch.einsum('tc,t->tc',
        #                                                                           concatenated_feature_e,
        #                                                                           action_clone_tensor)
        # length = concatenated_feature.size(0)
        # concatenated_feature_final[:, :length, :] = concatenated_feature
        # total_e = concatenated_feature[index_action[0]:(index_action[0] + Elen_x)]
        # e_feature.append(total_e)
        #
        # # Comframewise=concatenated_feature_final.transpose(1, 2)
        # input_lengths_entire_com = []
        # for i in range(batch):
        #     input_lengths_entire_com.append(Tlen_x + Elen_x)
        ...
        # Comframewise.append(concatenated_feature_final.transpose(1, 2))
        # Clen_x.append(input_lengths_entire_loca[0])
        Comframewise, input_lengths_entire_loca,location_prediction,length_T=self.G(Tframewise_all,Eframewise_all,Tlen_x_all,Elen_x_all)
        # Tlen_x = torch.zeros_like(Tlen_x_all).cuda()   #456-551放到一个对象里，当做生成器
        # Elen_x = torch.zeros_like(Elen_x_all).cuda()
        # # Clen_x = torch.zeros_like(Clen_x_all).cuda()
        # hand_start_T_list = []
        ...
        # for i in range(batch):
        #     input_lengths_entire_loca[i] = input_lengths_entire_com[i]
        conv1d_outputs_first = self.conv1d_combine_first(Comframewise.transpose(1, 2), input_lengths_entire_loca)
        # x: T, B, C
        x_first = conv1d_outputs_first['visual_feat']
        lgt_first = conv1d_outputs_first['feat_len']
        tm_outputs_first = self.temporal_model_combine_first(x_first, lgt_first)
        ...
        # decision=self.D(tm_outputs_second['predictions'])
        return {
            "framewise_features": Comframewise.transpose(1, 2),
            "visual_features": x_first,
            "feat_len": lgt_first,
            "conv_logits": conv1d_outputs_first['conv_logits'],
            "clipwise_features": tm_outputs_first['predictions'],
    ...
    def forward_train_d1(self, Tx):
        ...
        # label_calss=Variable(torch.ones([1]))
        # label_d.append(label_calss)
        decision=self.D.forward_classifier(Tx)
        return {
            # "framewise_features": Tframewise_all.transpose(1, 2),
            # "visual_features": x_second,
            # "feat_len": lgt_second,
            # "conv_logits": conv1d_outputs_second['conv_logits'],
            # "clipwise_features": tm_outputs_second['predictions'],
            # "sequence_logits": outputs,
            # "conv_sents": conv_pred,
            # "recognized_sents": pred,
            # "label_calss": label_d_new,
    ...
    def forward_train_d2(self, Tx):

        #
        outputs = self.D.forward_ctc(Tx["clipwise_features"])
        pred = self.decoder_combine.decode(outputs, Tx["feat_len"], batch_first=False, probs=False)
        # conv_pred = self.decoder_combine.decode(conv1d_outputs_second['conv_logits'], Tx["feat_len"], batch_first=False,
        #                                         probs=False)
        # decision = self.D(tm_outputs_second['predictions'])
        # label_calss=Variable(torch.ones([1]))
        # label_d.append(label_calss)
    ...
    def forward_train_d3(self, Tx):
        ...
        # label_calss=Variable(torch.ones([1]))
        # label_d.append(label_calss)
        location_d=self.D.forward_location(Tx)
        return {
            # "framewise_features": Tframewise_all.transpose(1, 2),
            # "visual_features": x_second,
            # "feat_len": lgt_second,
    ...
    def forward_train(self, Tx, Ex , Tlen_x_all, Elen_x_all, label=None, label_lgt=None,T_label=None, T_label_lgt=None,E_label=None,E_label_lgt=None,insert_word_before=None,insert_word_after=None,insert_frame_before=None,insert_frame_after=None,right_pad_list_T=None,right_pad_list_E=None):
        # videos
        Clen_x=[]
        # for i in range(len(Tlen_x_all)):
        #     if Tlen_x_all[i]==Elen_x_all[i]:
        #         length=Tlen_x_all[i]
        #         length_total.append(length)
        #     else:
        #         length=Tlen_x_all[i]+Elen_x_all[i]
        #         length_total.append(length)
        # Clen_x_max=max(length_total)
        # Clen_x=torch.tensor(length_total)
        Comframewise=[]
        label_d=[]
        for i in range(len(Tlen_x_all)):
            ...
            if Tlen_x_all[i]==Elen_x_all[i]:
                ...
                _, channel, height, width = Tx[i].shape
                # Clen_x_all = Tlen_x_all + Elen_x_all
                batch=1
                a = []
                a.append(Tlen_x_all[i])
                a_T = torch.tensor(a)
                Ttemp = torch.max(Tlen_x_all)
                a_max = []
                a_max.append(Ttemp)
                a_T_max = torch.tensor(a_max)
                Tinputs = Tx[i].reshape(batch * Ttemp, channel, height, width)
                Tframewise_all = self.masked_bn(Tinputs, a_T,a_T_max)
                Tframewise_all = Tframewise_all.reshape(batch, Ttemp, -1)
                Comframewise.append(Tframewise_all.transpose(1, 2))
                Clen_x.append(Ttemp)
                ...
                _, channel, height, width = Tx[i].shape
                _, _, _, _ = Ex[i].shape
                Clen_x_all = Tlen_x_all[i] + Elen_x_all[i]
                batch=1
                Ttemp=torch.max(Tlen_x_all)
                a_max = []
                a_max.append(Ttemp)
                ...
                Tframewise_all = Tframewise_all.reshape(batch, Ttemp, -1)
                b = []
                b.append(Elen_x_all[i])
                b_T = torch.tensor(b)
                Etemp=torch.max(Elen_x_all)
                b_max = []
                b_max.append(Etemp)
                b_T_max = torch.tensor(b_max)
                Einputs = Ex[i].reshape(batch * Etemp, channel, height, width)
                Eframewise_all = self.masked_bn(Einputs, b_T,b_T_max)
                Eframewise_all = Eframewise_all.reshape(batch, Etemp, -1)


                Tlen_x=torch.zeros_like(Tlen_x_all[i]).cuda()
                Elen_x=torch.zeros_like(Elen_x_all[i]).cuda()
                # Clen_x=torch.zeros_like(Clen_x_all[i]).cuda()
                hand_start_T_list=[]
                hand_end_T_list=[]
                hand_start_E_list=[]
                ...
                # for i in range(0,batch):
                hand_start_T,hand_end_T=self.cosine_similarity_onevideo(Tframewise_all[0],Tlen_x_all[i])
                hand_start_E,hand_end_E=self.cosine_similarity_onevideo(Eframewise_all[0],Elen_x_all[i])
                hand_start_T_list.append(hand_start_T)
                hand_start_E_list.append(hand_start_E)
                hand_end_T_list.append(hand_end_T)
                hand_end_E_list.append(hand_end_E)
                ...
                label_calss = Variable(torch.zeros([1]))
                label_d.append(label_calss)
                # decision = self.D(tm_outputs_second['predictions'])
        Clen_x_new=torch.zeros(2).cuda()
        label_d_new=torch.zeros(2,1).cuda()
        for i in range(Tx.size(0)):
            Clen_x_new[i]=Clen_x[i]
            label_d_new[i,:]=label_d[i]
        Clen_x_new=Clen_x_new.int()
        Comframewise_new=torch.zeros(Tx.size(0),512,torch.max(Clen_x_new)).cuda()
        for i in range(Tx.size(0)):
            Comframewise_new[i,:,:Clen_x_new[i]]=Comframewise[i][0]
        Comframewise=Comframewise_new
        Clen_x=Clen_x_new
        conv1d_outputs_first = self.conv1d_combine_first(Comframewise, Clen_x)
        # x: T, B, C
        x_first = conv1d_outputs_first['visual_feat']
    ...
    def forward_test(self, x, len_x_all, label=None, label_lgt=None):
        if len(x.shape) == 5:
            ...
            framewise_all = self.masked_bn(inputs, len_x_all,len_x_all)
    ...
    def criterion_calculation_d2(self, ret_dict, label, label_lgt):
        loss = 0
        # E_label_lgt=torch.ones(4)
        for k, weight in self.loss_weights.items():
            # if k == 'ConvCTC':
        ...
        return loss
    def criterion_calculation_d1(self, ret_dict, label):
        loss = 0
        # E_label_lgt=torch.ones(4)
        for k, weight in self.loss_weights.items():
            if k == 'cross':
                # if epoch>2:
                loss += weight * self.loss['cross_entropy'](ret_dict,
                                                      label).mean()

        return loss
    def criterion_calculation_d3(self, ret_dict, label,length):
        loss = 0
        # E_label_lgt=torch.ones(4)
        # ret_dict_length=ret_dict*length
        label_new=label/length
        # ret_dict_length=torch.floor(ret_dict_length)
        # label = label.to(torch.float32)
        for k, weight in self.loss_weights.items():
            if k == 'l1':
                # if epoch>2:
                loss += weight * (1-self.loss['ls_loss'](ret_dict,     #用l1或l2算距离，都需要0-1之间
                                                      label_new)).mean()
    ...
    def criterion_init(self):
        ...
        self.loss['cross_entropy']=nn.BCELoss()
        # self.loss['ls_loss'] = nn.L1Loss(size_average=None, reduce=None, reduction='none')
        self.loss['ls_loss'] = nn.L1Loss()
        # self.loss['ls_loss']=torch.nn.PairwiseDistance(p=2.0, eps=1e-06, keepdim=False)
        # self.loss['ls_loss']=nn.CosineSimilarity(dim=0, eps=1e-6)
        return self.loss
