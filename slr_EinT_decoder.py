...
class DecoderRNN(BaseRNN):
    ...
    def __init__(self, vocab_size, hidden_size, sos_id, eos_id, n_layers=1, rnn_cell='gru', bidirectional=False,
                 input_dropout_p=0, dropout_p=0, use_attention=False, use_Question=True,batch_size=None):
        ...
        if use_Question:
            self.rnn = self.rnn_cell(640, hidden_size, n_layers, batch_first=True, dropout=dropout_p)
        else:
            self.rnn = self.rnn_cell(128, hidden_size, n_layers, batch_first=True, dropout=dropout_p)
        ...
        self.embedding = nn.Embedding(self.output_size, 128)   ###以前的

        ###新增加的
        model_path='/ssd1/pretrained_models/baike_26g_news_13g_novel_229g.model.wv.vectors.npy'
        model_path1 = '/ssd1/pretrained_models/word2vec/word2vec/word2vec_wx.wv.syn0.npy'
        # word2vec = gensim.models.KeyedVectors.load_word2vec_format('/ssd1/pretrained_models/sgns.wiki.word', binary=True,encoding='utf-8')
        # wvmodel = gensim.models.KeyedVectors.load_word2vec_format(model_path, binary=False,encoding='utf-8')
        # loadData = np.load(model_path)
        # loadData=loadData[:self.output_size,:]
        # syn=np.load(model_path1)
        #
        # self.embedding = nn.Embedding(self.output_size, 128)
        # pretrained_weight = np.array(loadData)
        ...
        if use_attention:
            self.attention = Attention(self.hidden_size,self.batch_size)

        self.out = nn.Linear(self.hidden_size, self.output_size)
        self.max_length = 50

    def forward_step(self, input_var, hidden, encoder_outputs, function, q=None):
        ...
        embedded_s=embedded
        if q is not None:
            if embedded.size(1) != q.size(1):
                q = q.repeat(1, output_size, 1)
                embedded = torch.cat((embedded, q), dim=2)
            else:
                embedded = torch.cat((embedded, q), dim=2)
        ...
        return predicted_softmax, output, hidden, attn, context_feature,embedded_s

    def forward(self, inputs=None, input_lengths=None, encoder_hidden=None, encoder_outputs=None,
                function=F.log_softmax, teacher_forcing_ratio=0, cnn_outputs=None, q=None):
        ret_dict = dict()
        if self.use_attention:
        ...
        def decode(step, step_output, step_attn, step_context_feature):
            ...
            eos_batches = symbols.data.eq(self.eos_id)  ####判断当前预测的结果是否为结束符
            aa=eos_batches.dim()
        ...
        decoder_a_linear=torch.zeros(batch_size, max_length, self.output_size)
        if use_teacher_forcing:
            decoder_input = inputs[:, :-1]
            # print('decoder_input_teacher:',decoder_input.size())
            decoder_output, decoder_embedding, decoder_hidden, attn, context_feature,embedded_s = self.forward_step(decoder_input,
                                                                                                         decoder_hidden,
                                                                                                         encoder_outputs,
                                                                                                         function=function,
                                                                                                         q=q)
            decoder_embeddings = decoder_embedding[:, :-1, :]
            decoder_a_linear= decoder_output
            # print('decoder_output_teacher:',decoder_output.size())
            ...
            embedded_s = torch.zeros(batch_size, max_length,128)
            decoder_input = inputs[:, 0].unsqueeze(1)
            # print('decoder_input:',decoder_input.size())
            for di in range(max_length):
                decoder_output, decoder_embedding, decoder_hidden, step_attn, step_context_feature,embedded_s1 = self.forward_step(
                    decoder_input,
                    decoder_hidden,
                    encoder_outputs,
                    function=function,
                    q=q)
                embedded_s[:,di,:]=embedded_s1.squeeze()
                ...
                decoder_a_linear[:, di, :] = decoder_output.squeeze()
        ...
        return decoder_outputs, decoder_embeddings, decoder_hidden, ret_dict,embedded_s,decoder_a_linear
...
class SLRModel(nn.Module):
    def __init__(self, num_classes, num_classes_T,num_classes_E,c2d_type, conv_type, use_bn=False, tm_type='BiLSTM',
        ...
        # self.tau = 5
        self.ch_decoder_rnn=DecoderRNN(vocab_size, hidden_size, sos_id, eos_id, n_layers=1, rnn_cell='gru', bidirectional=False,
                 input_dropout_p=0, dropout_p=0, use_attention=False, use_Question=True,batch_size=None)
    ...
    def forward_train(self, Tx, Ex , Tlen_x_all, Elen_x_all, label=None, label_lgt=None,T_label=None, T_label_lgt=None,E_label=None,E_label_lgt=None,insert_word_before=None,insert_word_after=None,insert_frame_before=None,insert_frame_after=None,right_pad_list_T=None,right_pad_list_E=None):
        ...
        e_feature = []
        concatenated_feature_final = torch.zeros(batch, t_length + e_length, 512).cuda()
        ...
        # input_lengths_entire = []
        # for i in range(batch):
        #     # com=torch.cat((features_input[i],features_e_input[i]),dim=0)
        #     # input.append(com)
        #     input_lengths_entire.append(Tlen_x[i] + Elen_x[i])
        # # input = torch.cat(input, dim=0)
        # # input_lengths_entire=torch.cat(input_lengths_entire,dim=0)
        # input_lengths_entire = torch.IntTensor(input_lengths_entire).cuda()
        ...
        # Clen_x=Tlen_x + Elen_x


        # 对template做手语识别
        # conv1d_outputs_T_first = self.conv1d_T_first(Comframewise, Clen_x)
        # # conv1d_outputs_T_first = self.conv1d_T_first(Tframewise, Tlen_x)
        # # x: T, B, C
        ...
        # pred_T = None if self.training \
        #     else self.decoder_T.decode(outputs_T, lgt_T, batch_first=False, probs=False)
        # conv_pred_T = None if self.training \
        #     else self.decoder_T.decode(conv1d_outputs_T['conv_logits'], lgt_T, batch_first=False, probs=False)
        ...
        # feature disentangle
        # tm_outputs_T_first['predictions'] feature_T feature_E: bs*length*hidden_dim
        # feature_T,feature_E_complement,mask1=self.feature_disentangle_T(tm_outputs_T_first['predictions'].permute(1,2,0))
        # feature_E,feature_T_complement,mask2=self.feature_disentangle_E(tm_outputs_E_first['predictions'].permute(1,2,0))
        # feature fusion
        # tm_outputs_fusion=self.feature_fusion(torch.cat([feature_T,feature_E,tm_outputs_first['predictions']],dim=2))
        # tm_outputs_fusion=self.feature_fusion_activation(tm_outputs_fusion)
        # feature_fusion_T=self.attention_T(queries=tm_outputs_first['predictions'].permute(1,0,2), keys=feature_T.permute(1,0,2), values=feature_T.permute(1,0,2), valid_lens=lgt_T_first.cuda())
        # feature_fusion_E=self.attention_E(queries=tm_outputs_first['predictions'].permute(1,0,2), keys=feature_E.permute(1,0,2), values=feature_E.permute(1,0,2), valid_lens=lgt_E_first.cuda())
        # tm_outputs_fusion=self.layer_norm(tm_outputs_first['predictions']+feature_fusion_T.permute(1,0,2)+feature_fusion_E.permute(1,0,2))

        # feature_fusion_T=feature_T
        # feature_fusion_E=feature_E
        # tm_outputs_fusion=self.layer_norm(tm_outputs_first['predictions']+feature_fusion_T+feature_fusion_E)

        # tm_outputs_fusion = self.feature_fusion(torch.cat([feature_T, tm_outputs_first['predictions'], feature_E], dim=2))
        ...
        #         figure_name='/disk1/shipeng/vac_insert_multitask/只有首尾拼接_不共享参数_disentangle_3_test/20220124/'+str(random.randint(0,30))+'开头插入mask1'+'.png'
        ...
        #         plt.figure()    
        #         mask_to_print=mask1[i].cpu().detach().numpy()
        #         figure_name='/disk1/shipeng/vac_insert_multitask/只有首尾拼接_不共享参数_disentangle_3_test/20220124/'+str(random.randint(0,30))+'末尾插入mask1'+'.png'
        #         plt.plot(mask_to_print)
        #         plt.savefig(figure_name)
        #         plt.close()
        ...
        #         figure_name='/disk1/shipeng/vac_insert_multitask/只有首尾拼接_不共享参数_disentangle_3_test/20220124/'+str(random.randint(0,30))+'开头插入mask2'+'.png'
        #         plt.plot(mask_to_print)
        #         plt.savefig(figure_name)
        #         plt.close()
        #     # 在template末尾插入
        ...
        #         plt.figure()    
        #         mask_to_print=mask2[i].cpu().detach().numpy()
        #         figure_name='/disk1/shipeng/vac_insert_multitask/只有首尾拼接_不共享参数_disentangle_3_test/20220124/'+str(random.randint(0,30))+'末尾插入mask2'+'.png'
        #         plt.plot(mask_to_print)
        #         plt.savefig(figure_name)
        #         plt.close()
        ...
        # step 2 :
        # 对template做手语识别
        # feature_T=tm_outputs_T_first['predictions']
        # conv1d_outputs_T_second = self.conv1d_T_second(feature_T.permute(1,2,0), lgt_T_first)
        # # x: T, B, C
        # x_T_second = conv1d_outputs_T_second['visual_feat']
        # lgt_T_second = conv1d_outputs_T_second['feat_len']
        # tm_outputs_T_second = self.temporal_model_T_second(x_T_second, lgt_T_second)
        # outputs_T = self.classifier_T(tm_outputs_T_second['predictions'])
        ...
        q=None
        target_batch=label
        encoder_hidden=tm_outputs_second['hidden']
        encoder_output=tm_outputs_second['predictions']
        teacher_forcing_ratio=0
        decoder_outputs, decoder_embedding, decoder_hidden, other, embedded_s, decoder_a_linear = self.ch_decoder_rnn(
            inputs=target_batch,
            input_lengths=input_lengths_entire_loca,
            encoder_hidden=encoder_hidden,
            encoder_outputs=encoder_output,
        ...
        conv_output=tm_outputs_second['predictions']
        vis_pe = torch.arange(max(lgt_second), dtype=torch.long, device=conv_output.device)
        vis_pe = vis_pe.expand(conv_output.size(1), conv_output.size(0))
        mask_sign = torch.zeros(batch, max(lgt_second), max(lgt_second), dtype=torch.long, device=conv_output.device)
        for i in range(batch):
            mask_sign[i] = self.get_attn_mask(lgt_second[i], max(lgt_second))
        conv_output = conv_output.transpose(0, 1)
        text=None
        bert_output = self.bert(conv_output, vis_pe, text, attention_mask=mask_sign)
        bert_output_sign = bert_output
        trg_embed = self.text_embedding(None, None, label)    ##### label?????
        src_mask = torch.zeros(batch, max(lgt_second), max(lgt_second), dtype=torch.bool, device=conv_output.device)
        trg_mask = torch.zeros(batch, max(label_lgt), max(label_lgt), dtype=torch.bool,
                               device=conv_output.device)
        for i in range(batch):
            trg_mask[i] = self.get_attn_mask_boolean(label_lgt[i], max(label_lgt))    #####label_lgt?？
        decoder_outputs = self.decoder(
            trg_embed=trg_embed,
            encoder_output=bert_output_sign,  # 视频logits  b t p
    ...
    def forward_test(self, x, len_x_all, label=None, label_lgt=None):
        ...
        #E识别
        # conv1d_outputs_E_second = self.conv1d_E_second(feature_E.permute(1,2,0), lgt_E_first)
        # # x: T, B, C
        # x_E_second = conv1d_outputs_E_second['visual_feat']
        # lgt_E_second = conv1d_outputs_E_second['feat_len']
        # tm_outputs_E_second = self.temporal_model_E_second(x_E_second, lgt_E_second)
