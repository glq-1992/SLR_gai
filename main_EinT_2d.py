...
faulthandler.enable()
import utils
import utils.optimizer_d
# from seq_EinT import seq_train, seq_eval, seq_feature_generation
from seq_EinT_2d_te_fine import seq_train, seq_eval, seq_feature_generation ##seq_EinT_2d; seq_EinT_2d_te_fine
...
class Processor():
    def __init__(self, arg):
        ...
        # new label
        ch_vocab_filename = "DatasetFile/3004_new/vocab_esplit.txt"
        with open(ch_vocab_filename, encoding='utf-8') as f:
            vocab = f.readlines()
            self.gloss_dict = {vocab[i].replace('\n', ''):i+1 for i in range(len(vocab))} ### blank:0
            # self.gloss_dict = {vocab[i].replace('\n', ''): i  for i in range(len(vocab))}

        ch_vocab_filename_T = "DatasetFile/3004_new/vocab_esplit.txt"
        with open(ch_vocab_filename_T, encoding='utf-8') as f:
            vocab = f.readlines()
            self.gloss_dict_T = {vocab[i].replace('\n', ''):i+1 for i in range(len(vocab))}

        ch_vocab_filename_E = "DatasetFile/3004_new/vocab_esplit.txt"
        with open(ch_vocab_filename_E, encoding='utf-8') as f:
            vocab = f.readlines()
            self.gloss_dict_E = {vocab[i].replace('\n', ''):i+1 for i in range(len(vocab))}
        # new label
        ...
        self.model, self.optimizer, self.optimizer_d = self.loading()
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.optimizer_args['start_epoch'], self.arg.num_epoch):
                ...
                # train end2end model
                seq_train(self.data_loader['train'], self.model, self.optimizer,self.optimizer_d,self.device,
                          epoch, self.recoder)
                if eval_model:
    ...
    def loading(self):
        ...
        model = model_class(
            **self.arg.model_args,
            ...
            gloss_dict_T=self.gloss_dict_T,
            gloss_dict_E=self.gloss_dict_E,
            loss_weights=self.arg.loss_weights,
        )
        # optimizer = utils.Optimizer(model.G, self.arg.optimizer_args) #g
        optimizer = utils.Optimizer(model, self.arg.optimizer_args)    ######???????用list列出来
        # optimizer = utils.Optimizer([model.conv2d,model.conv1d_combine_first,model.temporal_model_combine_first], self.arg.optimizer_args)
        # optimizer_d = utils.Optimizer(model.D, self.arg.optimizer_args_d) #d
        optimizer_d = utils.optimizer_d.Optimizer_d(model.D, self.arg.optimizer_args_d)  # d
        ...
        self.load_data()
        return model, optimizer,optimizer_d
    ...
    #     pre_trained_dict = {k: v for k, v in state_dict['model_state_dict'].items() if
    #                         k in classifier_dict and v.size() == classifier_dict[k].size()}
    #     weights = self.modified_weights(pre_trained_dict, False)
    ...
    def load_data(self):
        ...
        for idx, (mode, train_flag) in enumerate(dataset_list):
            ...
            self.dataset[mode] = self.feeder(gloss_dict=self.gloss_dict,gloss_dict_T=self.gloss_dict_T,gloss_dict_E=self.gloss_dict_E, **arg)
