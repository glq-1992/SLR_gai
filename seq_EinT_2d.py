...
def seq_train(loader, model, optimizer, optimizer_d, device, epoch_idx, recoder):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        Cvid = device.data_to_device(data[2])
        Tvid_lgt = device.data_to_device(data[3])
        Evid_lgt = device.data_to_device(data[4])
        Cvid_lgt = device.data_to_device(data[5])
        label = device.data_to_device(data[6])
        label_lgt = device.data_to_device(data[7])

        T_label = device.data_to_device(data[8])
        T_label_lgt = device.data_to_device(data[9])

        E_label = device.data_to_device(data[10])
        E_label_lgt = device.data_to_device(data[11])

        insert_word_before=device.data_to_device(data[12])
        insert_word_after=device.data_to_device(data[13])

        insert_frame_before=data[14]
        insert_frame_after=data[15]
        file_info = data[18]
        ...
        # d_loss = loss_d2_true  ##### only recognition
        ...
        # g_loss = loss_d2_false1       ### only recognition
